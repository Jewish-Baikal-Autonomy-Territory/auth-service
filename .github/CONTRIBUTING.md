...
Thank you for your interest in contributing to **auth-service**! We welcome contributions from the community to help improve the project. Here are some guidelines to help you get started:
...
- **feat**: A new feature (e.g. `feat: add support for oauth2 authentication using external google id provider`)
- **fix**: A bug fix (e.g. `fix: correct user validation filter chain in <class method>`)
- **perf**: A code change that improves performance (e.g. `perf: optimize validation algorithm for better performace`)
